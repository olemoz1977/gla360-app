...
// Pavyzdinis klausimynas (galėsim vėliau pakeisti į tikrą GLA360 banką)
const QUESTIONS = [
...
export default function SelfAssessmentForm({ onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // (Pasirinktinai) Išsaugoti į Firebase:
      // Jeigu sukonfigūruosi firebase.js (žr. 3 skyrių), atkomentuok:
      /*
      const { ensureAnonAuth, saveSelfAssessment } = await import('../../firebase.js')
      const uid = await ensureAnonAuth()
      await saveSelfAssessment(uid, {
        answers,
        scores: res,
      ...
      */
      ...
      console.error(err)
      alert('Nepavyko išsaugoti. Patikrink Console.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: 760 }}>
      ...
      {QUESTIONS.map(q => (
        <div key={q.id} style={{ marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid #eee' }}>
          <div style={{ marginBottom: 8, fontWeight: 600 }}>
            [{q.dimension}] {q.text} {q.reverse && <em style={{ color: '#888', fontWeight: 400 }}>(invertuotas)</em>}
          </div>
      ...
      <button
        ...
        style={{
          padding: '10px 16px',
          borderRadius: 8, border: '1px solid #ddd', background: allAnswered ? '#111' : '#999',
          color: '#fff', cursor: allAnswered ? 'pointer' : 'not-allowed'
        }}
      >
        {submitting ? 'Saugoma...' : 'Pateikti'}
      </button>
